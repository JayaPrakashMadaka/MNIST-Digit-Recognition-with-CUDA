...
void convlayer(float* input, float* output,float* kernels,float* biases, int input_size, int output_size,int kernel_size,int inchannels,int outchannels,cudaStream_t stream) {
    int threads_per_block = T;
    int num_elements = outchannels * output_size * output_size;
    int num_blocks = (num_elements + threads_per_block - 1) / threads_per_block;
    convLayerKernel<<<num_blocks, threads_per_block,0,stream>>>(input, output, kernels, biases, input_size, output_size, kernel_size, inchannels, outchannels);
    cudaStreamSynchronize(stream);
}


void maxpoollayer(float* input, float* output, int input_size, int output_size,int inchannels,cudaStream_t stream) {
    int threads_per_block = T;
    int num_elements = output_size * output_size * inchannels;
    int num_blocks = (num_elements + threads_per_block - 1) / threads_per_block;
    maxpoolLayerKernel<<<num_blocks, threads_per_block,0,stream>>>(input, output, input_size, output_size, inchannels);
    cudaStreamSynchronize(stream);
}


void relulayer(float* input, int input_size,int inchannels,cudaStream_t stream) {
    int threads_per_block = T;
    int num_elements = input_size * input_size * inchannels;
    int num_blocks = (num_elements + threads_per_block - 1) / threads_per_block;
    reluLayerKernel<<<num_blocks, threads_per_block,0,stream>>>(input,input_size,inchannels);
    cudaStreamSynchronize(stream);
}
...
void streamscode() {
    ...
    if(dir){
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            float* input1;
            cudaMalloc(&input1, 28 * 28 * sizeof(float));
            cudaMemcpyAsync(input1, image, 28 * 28 * sizeof(float), cudaMemcpyHostToDevice,streams[ctr%num_streams]);

            float* output = (float*)malloc(10 * sizeof(float));
        
            convlayer(input1, output1, kernels1, biases1, 28, 24, 5, 1, 20,streams[ctr%num_streams]);
            maxpoollayer(output1, output2, 24, 12, 20,streams[ctr%num_streams]);
            convlayer(output2, output3, kernels2, biases2, 12, 8, 5, 20, 50,streams[ctr%num_streams]);
            maxpoollayer(output3, output4, 8, 4, 50,streams[ctr%num_streams]);
            convlayer(output4, output5, kernels3, biases3, 4, 1, 4, 50, 500,streams[ctr%num_streams]);
            relulayer(output5, 1, 500,streams[ctr%num_streams]);
            convlayer(output5, output6, kernels4, biases4, 1, 1, 1, 500, 10,streams[ctr%num_streams]);

            cudaMemcpyAsync(output, output6, 10 * sizeof(float), cudaMemcpyDeviceToHost,streams[ctr%num_streams]);
